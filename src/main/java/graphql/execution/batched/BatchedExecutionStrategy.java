...
public class BatchedExecutionStrategy extends ExecutionStrategy {
    ...
    private CompletionStage<ExecutionResult> execute(ExecutionContext executionContext, GraphQLExecutionNode root) {
        ...
        CompletableFuture.allOf(nodePromises.toArray(nodePromisesArray)).thenAccept(aVoid ->
                promise.complete(new ExecutionResultImpl(getOnlyElement(root.getData()).getParentResult(), executionContext.getErrors())));
        return promise;
    ...
    private CompletionStage<List<GraphQLExecutionNode>> resolveField(ExecutionContext executionContext, GraphQLObjectType parentType,
        ...
        return fetchData(executionContext, parentType, nodeData, fields, fieldDef)
                .thenCompose(values ->
                        CompletableFuture.completedFuture(completeValues(executionContext, parentType, values, fieldName, fields, fieldDef.getType())));
    }
    ...
    private CompletionStage<List<GraphQLExecutionNodeValue>> fetchData(ExecutionContext executionContext, GraphQLObjectType parentType,
        ...
        dataFetcher.get(environment)
                .thenApply(data -> (List<Object>) data)
                .exceptionally(e -> {
